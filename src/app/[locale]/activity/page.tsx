...
export default function ActivityPickerPage() {
  const t = useTranslations('activityPicker');
              ...
              try {
                ...
                if (r.ok) {
                  ...
                  if (Array.isArray(tasks)) {
                    const active = tasks.filter((task: {status: string}) =>
                      ['assigned', 'in_progress', 'testing', 'verification', 'convoy_active'].includes(
                        task.status
                      )
                    ).length;
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        {workspaces.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h2 className="text-xl font-bold text-mc-text mb-2">{t('emptyTitle')}</h2>
            <p className="text-mc-text-secondary">{t('emptyDescription')}</p>
          </div>
        ) : (
                    ...
                    <div className="flex gap-4 text-xs text-mc-text-secondary">
                      {counts.active > 0 && (
                        <span className="flex items-center gap-1">
                          ...
                          {t('activeTasks', {count: counts.active})}
                        </span>
                      )}
                      <span>{t('totalTasks', {count: counts.total})}</span>
                    </div>
                  )}
                </Link>
              );
